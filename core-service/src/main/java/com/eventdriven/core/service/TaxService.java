...
import com.eventdriven.core.client.EventRequest;
import com.eventdriven.core.client.EventServiceClient;
import com.eventdriven.core.dto.request.CreateTaxProfileRequest;
import com.eventdriven.core.dto.request.CreateUserRequest;
...
import com.eventdriven.core.dto.response.UserResponse;
import com.eventdriven.core.dto.response.WorkDayEntryResponse;
import com.eventdriven.core.entity.*;
import com.eventdriven.core.repository.*;
 ...
 * Publishes events to event-service via Feign Client (HTTP over Eureka).
 */
@Service
public class TaxService {
    ...
    private void publishEvent(String userId, String resourceId, String eventType, String details) {
        try {
            eventServiceClient.recordEvent(new EventRequest(userId, resourceId, eventType, details));
        } catch (Exception e) {
            log.warn("Failed to publish event to event-service: {}", e.getMessage());
        }
    }
    ...
    public UserResponse createUser(CreateUserRequest request) {
        ...
        User user = new User(request.getEmail(), passwordHash,
                request.getFirstName(), request.getLastName(), request.getSsnEncrypted());
        User saved = userRepository.save(user);
        publishEvent(saved.getUserId().toString(), saved.getUserId().toString(),
                "USER_CREATED", "User created: " + saved.getFirstName() + " " + saved.getLastName());
    ...
    public List<UserResponse> searchUsers(String term) {
        return userRepository.search(term).stream().map(UserResponse::from).collect(Collectors.toList());
    }
    ...
    public TaxProfileResponse createTaxProfile(CreateTaxProfileRequest request) {
        ...
        publishEvent(user.getUserId().toString(), saved.getProfileId().toString(),
                "TAX_PROFILE_CREATED", "Tax profile created for year " + saved.getTaxYear() + " in " + state.getStateCode());
    ...
    public List<TaxProfileResponse> findTaxProfilesByUser(UUID userId) {
        return taxProfileRepository.findByUser_UserId(userId).stream().map(TaxProfileResponse::from).collect(Collectors.toList());
    }
    ...
    public WorkDayEntryResponse createWorkDayEntry(CreateWorkDayEntryRequest request) {
        ...
        WorkDayEntry entry = new WorkDayEntry(profile, state, request.getStartDate(),
                request.getEndDate(), request.getIncome(), workType);
        WorkDayEntry saved = workDayEntryRepository.save(entry);
        publishEvent(profile.getUser().getUserId().toString(), saved.getEntryId().toString(),
                "WORK_ENTRY_CREATED", "Work entry created: " + workType + " in " + state.getStateCode());
    ...
    public List<WorkDayEntryResponse> findEntriesByProfile(UUID profileId) {
        return workDayEntryRepository.findByTaxProfile_ProfileId(profileId).stream()
                .map(WorkDayEntryResponse::from).collect(Collectors.toList());
    }
